        ...
        <div
            ...
            <div style="flex: 1; min-width: 300px;">
                ...
                <p style="color: #666; font-size: 1.1rem; max-width: 700px;">
                    Providing community-driven recipe cards, cooking tutorials, and instructional videos on various
                    cooking techniques and kitchen hacks.
                </p>
            </div>
            <div>
                <?php if (isset($_SESSION['user_id'])): ?>
                    <button onclick="document.getElementById('uploadResourceModal').style.display='flex'"
                        class="btn primary-btn" style="border-radius: 30px; box-shadow: 0 4px 15px rgba(230,57,70,0.3);">+
                        Add Resource</button>
                <?php else: ?>
                    <button onclick="alert('Please Login or Join Us to upload a resource!')" class="btn outline-btn"
                        style="border-radius: 30px;">+ Add Resource</button>
                <?php endif; ?>
            </div>
        </div>
                    ...
                    <div class="card resource-card">
                        ...
                        <div class="card-content" style="text-align: center;">
                            <span class="badge <?php echo $badgeClass; ?>"
                                style="margin-bottom: 10px; display: inline-block; <?php echo ($type != 'pdf' ? "background: $btnColor; color: white;" : ""); ?>"><?php echo $badgeText; ?></span>
                            <p
    ...
    <?php if (isset($_SESSION['user_id'])): ?>
        <div id="uploadResourceModal" class="modal" style="align-items: center; justify-content: center; z-index: 1000;">
            <div class="modal-content" style="max-width: 500px; padding: 40px; border-radius: 15px;">
                <span class="close-btn" onclick="document.getElementById('uploadResourceModal').style.display='none'"
                    style="position:absolute; top:20px; right:25px;">&times;</span>
                <h2 style="font-family: var(--font-heading); color: var(--primary-color); margin-bottom: 5px;">Share a
                    Resource</h2>
                ...
                <form action="submit_resource.php" method="POST" enctype="multipart/form-data">
                    ...
                    <input type="text" name="title" placeholder="Resource Title (e.g. Knife Skills 101)" required
                        style="width: 100%; margin-bottom: 15px; padding: 12px; border-radius: 8px; border: 1px solid #ccc;">

                    <input type="text" name="description" placeholder="Category Tag (e.g. Cooking Technique)" required
                        style="width: 100%; margin-bottom: 15px; padding: 12px; border-radius: 8px; border: 1px solid #ccc;">

                    <select name="type" id="resourceTypeSelect_cul" onchange="toggleResourceInput('cul')" required
                    ...
                    <div id="urlInputContainer_cul" style="display: none; margin-bottom: 15px;">
                        <input type="url" name="resource_url" id="resourceUrl_cul" placeholder="https://example.com/..."
                    ...
                    <div id="fileInputContainer_cul" style="display: none; margin-bottom: 15px;">
                        <label style="display: block; margin-bottom: 8px; color: #555; font-weight: 500;">Select PDF
                            File:</label>
                        <input type="file" name="resource_file" id="resourceFile_cul" accept="application/pdf"
                    ...
                    <button type="submit" class="btn primary-btn full-width"
                        style="border-radius: 30px; font-weight: 600; padding: 14px; margin-top: 5px;">Upload
                        Resource</button>
                </form>
            </div>
        </div>
