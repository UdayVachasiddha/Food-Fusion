...
<body>
    ...
    <header class="hero"
        style="height: 20vh; background: linear-gradient(rgba(29, 53, 87, 0.9), rgba(29, 53, 87, 0.9));">
    ...
    <main style="max-width: 800px; margin: 4rem auto; padding: 0 20px;">
        <div style="margin-bottom: 2rem;">
            <a href="community.php" style="color: var(--primary-color); text-decoration: none; font-weight: 600;">&larr;
                Back to Submission Form</a>
        </div>

        <?php if ($feed_result && $feed_result->num_rows > 0): ?>
            <?php while ($post = $feed_result->fetch_assoc()): ?>
                <div class="post-card"
                    style="background: #fff; padding: 25px; border-radius: 12px; box-shadow: 0 4px 15px rgba(0,0,0,0.05); margin-bottom: 25px; border-left: 5px solid var(--primary-color);">

                    <div
                        style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px; flex-wrap: wrap; gap: 10px;">
                        <div>
                            <span
                                style="background: #fef2f2; color: #dc2626; padding: 4px 12px; border-radius: 20px; font-size: 0.8rem; font-weight: 700; text-transform: uppercase;">
